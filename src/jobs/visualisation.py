...
new_column_names = [col_name.replace(' ', '_')
                    .replace('/', '')
                    .replace('.', '')
                    .replace(',', '') for col_name in df.columns]
...
fig = px.bar(df_continent, x='year', y='visa_issued', color='continent', barmode='group')
...
df_country_year_map.toPandas()

fig = px.choropleth(df_country_year_map, locations='country', color='visa_issued',
                    hover_name='country', animation_frame='year', range_color=[100000, 100000],
                    color_continuous_scale=px.colors.sequential.Plasma,
                    locationmode='country names', title='Yearly visa issued by countries')
